...
var reservedKeywordRegex = new RegExp(`^${RESERVED_KEYWORDS.join("|")}$`);
var identifierNameRegex = /^(?:[$_\p{ID_Start}])(?:[$_\u200C\u200D\p{ID_Continue}])*$/u;
var validIdentifierRegex = new RegExp(`(?!(${reservedKeywordRegex.source})$)${identifierNameRegex.source}`, "u");
var isValidIdentifer = (identifier) => validIdentifierRegex.test(identifier);
var normalizeIdentifier = (identifier) => identifier.replace(/(?:^[^$_\p{ID_Start}])|[^$_\u200C\u200D\p{ID_Continue}]/gu, "_");

// routes.ts
var HTTP_METHODS = ["HEAD", "OPTIONS", "GET", "POST", "PUT", "PATCH", "DELETE"];
function isHTTPMethod(maybeHTTPMethod) {
  return HTTP_METHODS.includes(maybeHTTPMethod);
}
async function writeRoutesModule({ config, srcDir, outfile = "_routes.js" }) {
...
function parseConfig(config, baseDir) {
  ...
  function parseModuleIdentifiers(paths) {
    ...
    return paths.map((modulePath) => {
      ...
      if (name2 !== "default" && !isValidIdentifer(name2)) {
        throw new Error(`Invalid module identifier "${name2}"`);
      }
      if (!identifier) {
        ...
        importMap.set(modulePath, { filepath: resolvedPath, name: name2, identifier });
      }
      return identifier;
    });
  }
...
function generateRoutesModule(importMap, routes) {
  return `${[...importMap.values()].map(({ filepath, name: name2, identifier }) => `import { ${name2} as ${identifier} } from "${filepath}"`).join("\n")}
...
// package.json
var name = "@cloudflare/pages-functions-compiler";
var version = "0.3.3";
var description = "";
var main = "index.ts";
var directories = {
...
var scripts = {
  build: "esbuild cli.ts --format=esm --platform=node --bundle --external:esbuild --external:commander --outfile=bin/plinko.mjs",
  "example:config": "yarn build && ./bin/plinko.mjs -c examples/config-routing/pages.json examples/config-routing",
  "example:filepath": "yarn build && ./bin/plinko.mjs examples/filepath-routing",
  prepublishOnly: "yarn build",
  test: "jest"
};
var bin = {
  plinko: "bin/plinko.mjs"
};
var files = [
  "bin",
  "templates"
];
var author = "Cina Saffary <cina@cloudflare.com>";
var license = "ISC";
var dependencies = {
  "@types/acorn": "^4.0.6",
  "@types/jest": "^27.0.2",
...
pp$6.finishNode = function(node, type2) {
  return finishNodeAt.call(this, node, type2, this.lastTokEnd, this.lastTokEndLoc);
};
pp$6.finishNodeAt = function(node, type2, pos, loc) {
...
pp$7.updateContext = function(prevType) {
  ...
  if (type2.keyword && prevType === types.dot) {
    this.exprAllowed = false;
...
pp$9.finishToken = function(type2, val) {
  this.end = this.pos;
  if (this.options.locations) {
...
pp$9.finishOp = function(type2, size) {
...
pp$9.readWord = function() {
  ...
  var type2 = types.name;
  if (this.keywords.test(word)) {
    type2 = keywords$1[word];
  }
...
function simple(node, visitors, baseVisitor, state, override) {
  ...
  (function c(node2, st, override2) {
    var type2 = override2 || node2.type, found = visitors[type2];
...
// filepath-routing.ts
async function generateConfigFromFileTree({ baseDir, baseURL }) {
  const routeEntries = [];
  ...
  await forEachFile(baseDir, async (filepath) => {
    ...
    if (/\.(mjs|js|ts)/.test(ext)) {
      const { code } = await transform(await fs2.readFile(filepath, "utf-8"), {
        loader: ext === ".ts" ? "ts" : "js"
      });
      const ast = parse3(code, { ecmaVersion: "latest", sourceType: "module" });
      simple(ast, {
        ExportNamedDeclaration(_node) {
          ...
          if (node.declaration) {
            ...
            if (declaration.type === "VariableDeclaration") {
              exportNames.push(...declaration.declarations.map((variableDeclarator) => variableDeclarator.id.name));
            }
          }
          if (node.specifiers.length) {
            exportNames.push(...node.specifiers.map((exportSpecifier) => exportSpecifier.exported.name));
          }
          for (const exportName of exportNames) {
            const [match, method] = exportName.match(/^onRequest(Get|Post|Put|Patch|Delete|Options|Head)?$/) ?? [];
            if (match) {
              const basename = path3.basename(filepath).slice(0, -ext.length);
              const isIndexFile = basename === "index";
              const isMiddlewareFile = basename === "_middleware" || basename === "_middleware_";
              let routePath = path3.relative(baseDir, filepath).slice(0, -ext.length);
              if (isIndexFile || isMiddlewareFile) {
                routePath = path3.dirname(routePath);
              }
              if (routePath === ".") {
                routePath = "";
              }
              ...
              routeEntries.push([
                routePath,
                {
                  [isMiddlewareFile ? "middleware" : "module"]: [
                    `${path3.relative(baseDir, filepath)}:${exportName}`
                  ]
                }
              ]);
            }
  ...
  return { routes: Object.fromEntries(routeEntries) };
}
function compareRoutes(a, b) {
  function parseRoutePath(routePath) {
    let [method, path5] = routePath.split(" ");
    if (!path5) {
      path5 = method;
      method = null;
    }
    const segments = path5.slice(1).split("/");
...
async function forEachFile(baseDir, fn) {
  const searchPaths = [baseDir];
  const returnValues = [];
  while (searchPaths.length) {
    let cwd = searchPaths.shift();
    const dir = await fs2.readdir(cwd, { withFileTypes: true });
    for (const entry of dir) {
      const pathname = path3.join(cwd, entry.name);
...
program.version(package_default.version).showHelpAfterError();
...
program.command("build", { isDefault: true }).description("build a Pages User Worker from a set of handler modules").argument("<directory>", "base directory of handler modules. If not using a config file, this path is used as the base for filepath-based routing.").option("-o, --outfile <filepath>", "path to output file").option("-c, --config <filepath>", "path to a config file containing route mappings").option("-m, --minify", "minify the output").option("-R, --outputConfig <filepath>", "path to output JSON file of config").option("-b, --baseURL <path>", "path to use as a base URL for all routes").action(build2);
async function build2(baseDir, {
  minify = false,
  config: configPath,
  ...
  const tmpDir = await fs3.mkdtemp(path4.join(os.tmpdir(), "plinko-"));
  const routesModule = path4.join(tmpDir, "routes.mjs");
  ...
  if (outputConfig) {
    await fs3.writeFile(outputConfig, JSON.stringify({ ...config, baseURL }, null, 2));
  }
}
program.parse();
